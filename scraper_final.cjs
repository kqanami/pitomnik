...
(async () => {
  ...
  let results = [];
  try {
      results = JSON.parse(fs.readFileSync('final_data2.json'));
  } catch(e) {}
  ...
  for (let url of [...new Set(LINKS)]) {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        // Target t772__content which we just verified holds the Tuya elements!
        const cards = Array.from(document.querySelectorAll('.t772__content, .t-store__card, .t-item')).map(card => {
           let name = card.querySelector('.t-name, .t-store__card__title, .t772__title')?.innerText?.trim();
           
           const desc = card.querySelector('.t-descr, .t-text, .t-store__card__descr, .t772__descr')?.innerText?.trim() || '';
           
           let img = card.querySelector('img')?.getAttribute('data-original') || card.querySelector('img')?.src;
           if (!img) {
               const imgDiv = card.querySelector('.t-bgimg, .t772__bgimg');
               if (imgDiv) {
  ...
  fs.writeFileSync('final_data3.json', JSON.stringify(results, null, 2));
  console.log('Saved to final_data3.json');
  await browser.close();
})();
