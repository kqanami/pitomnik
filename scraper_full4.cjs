  ...
  for (let url of [...new Set(LINKS)]) {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        const cards = Array.from(document.querySelectorAll('.t-item')).map(card => {
           const name = card.querySelector('.t-name, .t-title')?.innerText?.trim();
           const desc = card.querySelector('.t-descr, .t-text')?.innerText?.trim();
