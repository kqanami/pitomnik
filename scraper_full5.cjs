...
(async () => {
  ...
  for (let url of [...new Set(LINKS)]) {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        // This targets all Tilda grid components
        const cards = Array.from(document.querySelectorAll('.t-item, .t143__col, .t786__col, .t-store__card, .t776__col, .t772__wrapper')).map(card => {
           const name = card.querySelector('.t-name, .t-store__card__title, .t776__title, .t143__title, .t772__title')?.innerText?.trim();
           ...
           if (!img) {
               const imgDiv = card.querySelector('.t772__imgwrapper, .t-bgimg');
               if (imgDiv) {
  ...
  fs.writeFileSync('final_data.json', JSON.stringify(results, null, 2));
  console.log('Saved to final_data.json');
  await browser.close();
})();
